...
export default function RegistrationView({ regData, setRegData, handleRegister, loading }) {
  ...
  // Logic to determine if we should show commercial vehicle dimensions
  const requiresDimensions = regData.vehicleType && regData.vehicleType !== '2wheeler';
  const hasDimensionsSelected = regData.vehicleWeight && regData.vehicleDimensions;
        ...
        <form onSubmit={handleRegister} className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                    {vehicleTypes.map(v => (
                        <button key={v.id} type="button" onClick={() => updateField('vehicleType', v.id)}
                ...
                {/* DYNAMIC VEHICLE DIMENSIONS (Shows if Auto/Tempo/Truck is selected) */}
                {requiresDimensions && (
                    <div className="space-y-4 slide-down border-t border-slate-100 dark:border-slate-700 pt-4 mt-2">
                        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Load Capacity & Dimensions</p>
                        <div className="grid grid-cols-2 gap-3">
                            <select required className="reg-input w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border-2 border-transparent focus:border-brand-500 outline-none dark:text-white appearance-none"
                                value={regData.vehicleWeight || ''} onChange={(e) => updateField('vehicleWeight', e.target.value)}>
                                <option value="" disabled>Max Weight</option>
                                <option value="Up to 500 kg">Up to 500 kg</option>
                                <option value="500 kg - 1 Ton">500 kg - 1 Ton</option>
                                <option value="1 Ton - 2 Ton">1 Ton - 2 Ton</option>
                                <option value="Above 2 Ton">Above 2 Ton</option>
                            </select>

                            <select required className="reg-input w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border-2 border-transparent focus:border-brand-500 outline-none dark:text-white appearance-none"
                                value={regData.vehicleDimensions || ''} onChange={(e) => updateField('vehicleDimensions', e.target.value)}>
                                <option value="" disabled>Load Length</option>
                ...
                {/* DYNAMIC BODY TYPE (Shows after dimensions are picked) */}
                {requiresDimensions && hasDimensionsSelected && (
                    <div className="space-y-2 slide-down pt-2">
                        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Vehicle Body Type</p>
                        <div className="grid grid-cols-2 gap-3">
