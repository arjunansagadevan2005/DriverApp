...
export default function EarningsSection({ t, regData }) {
    ...
    // 🔥 FETCH REAL COMPLETED TRIPS FROM SUPABASE
    useEffect(() => {
        const fetchTrips = async () => {
            ...
            const { data, error } = await supabase
                .from('orders') // Search the orders table
                .select('*')
                .eq('driver_number', regData.mobile) // Match your driver number
                .ilike('status', 'Completed') // ONLY show completed deliveries
                .order('created_at', { ascending: false }); // Newest first

            if (data && !error) {
                // Format DB data to match your UI structure
    ...
    const total = filteredTrips.reduce((acc, curr) => acc + Number(curr.amount), 0);
    ...
    return (
        <div className="fade-in pt-4 pb-8 px-2">
            ...
            <div className="space-y-3 px-2">
                {isLoading ? (
                    <div className="text-center py-10 text-brand-600 dark:text-brand-400">Loading trips...</div>
